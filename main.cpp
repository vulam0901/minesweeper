...
void mainloop(Point2D  *food,int &score,float &difficult_level,int &heart,float &time) {
	if (GetAsyncKeyState(VK_LEFT)) {							// bat phím lên, xuông,trái,phai
        if (direction.x != 1) {
	...
	for(int i=3;i<snakelenght-1;i++)
		if (i != 0 && (snake[0].x == snake[i].x && snake[0].y == snake[i].y)) heart--;	//kiem tra xem dau ran co can trúng thân không
	if(time<0)
	{
			time=15; // refrest time
			heart--;	
	};
	if(heart<0) endGame= true; // dieu kiên endGame
	
	
	if(snake[0].x==(food+1)->x&& snake[0].y==(food+1)->y) //kiem tra dau ran co an trúng food_lv2 hay không
	{
    ...
    if(snake[0].x==food->x&& snake[0].y==food->y) // kiem tra dau ran co an trúng food_lv1 hay không
	{
		printf("%c",7);		// tao tiêng kêu
		score+=1;			// tang diêm
    	snakelenght+=1;		// tang dô dài	
    	time=15;			// refrest time
    	if(difficult_level>=20) 		// tao thêm dô khó cho game( cang nhiêu diem cang khó)
		{
		...
		if((score-5)%10==0) draw_food_lv2(food+1,console_wide,console_height); // tao food_lv2 khi diêm là 5,15,125,35,45...
	}
...
int main(){
	...
	while(!endGame){
		...
		mainloop(&food[0],score,difficult_level,heart,time);	//vòng lap xu lý logic game
